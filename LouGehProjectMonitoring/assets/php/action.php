    		...
    		if($row['status'] == 1){
    			$output .= '<td>'.$row['projname'].'</td>
		    		...
		    		<td align="right">
		    			...
		    			<button type="button" class="btn btn-primary editproj" id="'.implode("=",$row).'" data-toggle="modal" data-target="#upd_proj" style="font-size: .60em;"><i class="fas fa-edit"></i></button>

		    			<a class="btn btn-danger del_proj" title="delete" id="'.implode("=",$row).'" style="font-size: .60em;"><i class="fas fa-trash-alt"></i></a>
		    		</td>';
    		} else {
    			$output .= '<td>'.$row['projname'].'</td>
		    		<td>'.$row['deptname'].'</td>
